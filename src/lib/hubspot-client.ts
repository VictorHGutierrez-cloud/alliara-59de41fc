...
export async function startHubSpotOAuth(): Promise<{ authorizeUrl: string }> {
  ...
  if (!d?.authorizeUrl) throw new Error(d?.error ?? "No authorize URL");
...
export async function synthesizeHubSpotDigest(body: {
  ...
  if (!d?.ok || d.digest == null) throw new Error(d?.error ?? "Synthesis failed");
  return {
    ok: true,
    digest: d.digest,
...
export async function writeHubSpotCompanyNote(params: {
  ...
  if (!d?.ok) throw new Error(d?.error ?? d?.detail ?? "Write failed");
