...
function PartnerDiagnostic() {
  ...
  const submit = async () => {
    if (Object.keys(answers).length < total) {
      toast.error(COPY.diagnostic.answerAllQuestions);
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.success(COPY.diagnostic.savedToast);
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-2 h-1.5 rounded-full bg-surface-2 overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-[var(--octa-1)] via-[var(--octa-4)] to-[var(--octa-5)] transition-all"
        ...
        <div className="mt-6 space-y-2">
          {step.options.map((opt, i) => {
            ...
            return (
              ...
              >
                <span
                  className="h-6 w-6 shrink-0 rounded-full border border-border/80 flex items-center justify-center text-xs font-mono"
                  style={
                    selected
                      ? {
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        {!isLast ? (
          ...
          >
            {submitting ? "Saving…" : COPY.diagnostic.saveCta}
          </button>
        )}
      </div>
    </div>
